 ...
 * Change Logs:
 * Date           Author  Notes
 * 2018-08-06     limian  first version
 * 2018-10-10     limian  调试完成
 * 2018-12-04  limian  增加对64位卡号的上传
 *
 * note：  	
 */
...
#include "handlers.h"
#include "timestamp.h"
#include "app_flc_history.h"

#define CARD_HISTORY_READ_ONCE_MAX      10
...
void Flc_Card_History_Property_Lists(
    const int **pRequired,
    const int **pOptional,
...
bool Flc_Card_History_Valid_Instance(
...
unsigned Flc_Card_History_Count(
    void)
{
    return 1;
}
...
uint32_t Flc_Card_History_Index_To_Instance(
    unsigned index)
{
    return index;
}

unsigned Flc_Card_History_Instance_To_Index(uint32_t object_instance)
{
    return object_instance;
}

bool Flc_Card_History_Object_Name(
    uint32_t object_instance,
    BACNET_CHARACTER_STRING * object_name)
{
	...
	sprintf(text_string, "FLC Card History");
...
int Flc_Card_History_Read_Property(
    BACNET_READ_PROPERTY_DATA * rpdata)
{
	...
	uint32_t i;
    uint32_t start_no;
    struct str_card_history history[CARD_HISTORY_READ_ONCE_MAX];
    uint32_t count = 1;
    ...
    switch ((int) rpdata->object_property) {
        case PROP_OBJECT_IDENTIFIER:
            apdu_len = encode_application_object_id(&apdu[0], OBJECT_FLCHISTORY,
								rpdata->object_instance + 1);
            break;
        case PROP_OBJECT_NAME:
        case PROP_DESCRIPTION:
            Flc_Card_History_Object_Name(rpdata->object_instance, &char_string);
            apdu_len = encode_application_character_string(&apdu[0], &char_string);
            break;
        case PROP_OBJECT_TYPE:
            apdu_len = encode_application_enumerated(&apdu[0], OBJECT_FLCHISTORY);
            break;
            
        case PROP_FLCHISTORYDATA:
            
            /* 读取20条记录 */
            count = read_flc_history_record(history, start_no, CARD_HISTORY_READ_ONCE_MAX);
            if(count == 0)      /* 已经读完 */ 
            {
                break;
            }
...
bool Flc_Card_History_Write_Property(
