/*
 * File      : bac_door_schedule.c
 * function  : bacnet door_schedule 门时间表
 * Change Logs:
 * Date         Author  Notes
 * 2018-06-27   limian  create object
 * 2018-10-12   limian  调试完成，可以正常写入门时间表
 */
#include <stdbool.h>
#include <stdint.h>
#include <stdio.h>
...
#ifndef MAX_FLC_DOOR_SCHEDULE
#define MAX_FLC_DOOR_SCHEDULE MAX_DOOR_SCHEDULE_COUNT
#endif
...
void Flc_Door_Schedule_Property_Lists(
    const int **pRequired,
    const int **pOptional,
...
void Flc_Door_Schedule_Init(void)
{
    ...
    for (i = 0; i < MAX_FLC_DOOR_SCHEDULE; i++) {
	/* 变量初始化 */
...
bool Flc_Door_Schedule_Valid_Instance(
    uint32_t object_instance)
{
    unsigned int index;

    index = Flc_Door_Schedule_Instance_To_Index(object_instance);
    if (index < MAX_FLC_DOOR_SCHEDULE)
        return true;
...
unsigned Flc_Door_Schedule_Count(
    void)
{
    return MAX_FLC_DOOR_SCHEDULE;
}


uint32_t Flc_Door_Schedule_Index_To_Instance(
    unsigned index)
{
    if(index < MAX_FLC_DOOR_SCHEDULE)
        return index + 1;
...
unsigned Flc_Door_Schedule_Instance_To_Index(uint32_t object_instance)
{
    unsigned index = MAX_FLC_DOOR_SCHEDULE;

    if ((object_instance <= MAX_FLC_DOOR_SCHEDULE) && 
        (object_instance > 0))
        index = object_instance - 1;
    return index;
}

bool Flc_Door_Schedule_Object_Name(
    uint32_t object_instance,
    BACNET_CHARACTER_STRING * object_name)
{
    ...
    index = Flc_Door_Schedule_Instance_To_Index(object_instance);
    if (index < MAX_FLC_DOOR_SCHEDULE) {
        sprintf(text_string, "DOOR SCHEDULE");
...
int Flc_Door_Schedule_Read_Property(
...
//    BACNET_TIME time = {11, 25, 0, 0};
    ...
    object_index = Flc_Door_Schedule_Instance_To_Index(rpdata->object_instance);
    if (object_index >= MAX_FLC_DOOR_SCHEDULE)
        return BACNET_STATUS_ERROR;

    apdu = rpdata->application_data;
    switch ((int) rpdata->object_property) {
        case PROP_OBJECT_IDENTIFIER:
            apdu_len = encode_application_object_id(&apdu[0], OBJECT_DOORSCHEDULE,
								rpdata->object_instance + 1);
            break;
        case PROP_OBJECT_NAME:
        case PROP_DESCRIPTION:
            Flc_Door_Schedule_Object_Name(rpdata->object_instance, &char_string);
            apdu_len = encode_application_character_string(&apdu[0], &char_string);
            break;
        case PROP_OBJECT_TYPE:
            apdu_len = encode_application_enumerated(&apdu[0], OBJECT_DOORSCHEDULE);
            break;
...
bool Flc_Door_Schedule_Write_Property(
...
//	
	object_index = Flc_Door_Schedule_Instance_To_Index(wp_data->object_instance);
	if (object_index >= MAX_FLC_DOOR_SCHEDULE) {
        rt_free(value);
		return false;
	}
...
//            rt_kprintf("door_schedule write start****************\r\n");
...
//                        schedule.day[i].mode[j].start_time.minute, 
//                        schedule.day[i].mode[j].end_time.hour, 
//                        schedule.day[i].mode[j].end_time.minute);
//                }
//            }
//            rt_kprintf("end**************************************\r\n\r\n");
            app_schedule_write(&schedule, schedule_no, 1, SCHEDULE_TYPE_DOOR);
            status = true;
            break;
		case PROP_OBJECT_IDENTIFIER:
		case PROP_OBJECT_NAME:
