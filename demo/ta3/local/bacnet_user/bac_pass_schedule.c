...
#ifndef MAX_FLC_PASS_SCHEDULE
#define MAX_FLC_PASS_SCHEDULE MAX_PASS_SCHEDULE_COUNT
#endif
...
void Flc_Pass_Schedule_Property_Lists(
    const int **pRequired,
    const int **pOptional,
...
void Flc_Pass_Schedule_Init(void)
{
    ...
    for (i = 0; i < MAX_FLC_PASS_SCHEDULE; i++) {
	/* 变量初始化 */
...
bool Flc_Pass_Schedule_Valid_Instance(
    uint32_t object_instance)
{
    unsigned int index;

    index = Flc_Pass_Schedule_Instance_To_Index(object_instance);
    if (index < MAX_FLC_PASS_SCHEDULE)
        return true;
...
unsigned Flc_Pass_Schedule_Count(
    void)
{
    return MAX_FLC_PASS_SCHEDULE;
}


uint32_t Flc_Pass_Schedule_Index_To_Instance(
    unsigned index)
{
    if(index < MAX_FLC_PASS_SCHEDULE)
        return index + 1;
...
unsigned Flc_Pass_Schedule_Instance_To_Index(uint32_t object_instance)
{
    unsigned index = MAX_FLC_PASS_SCHEDULE;

    if ((object_instance <= MAX_FLC_PASS_SCHEDULE) &&
        (object_instance > 0))
...
bool Flc_Pass_Schedule_Object_Name(
    uint32_t object_instance,
    BACNET_CHARACTER_STRING * object_name)
{
    ...
    index = Flc_Pass_Schedule_Instance_To_Index(object_instance);
    if (index < MAX_FLC_PASS_SCHEDULE) {
        sprintf(text_string, "PASS SCHEDULE");
...
int Flc_Pass_Schedule_Read_Property(
...
//    BACNET_TIME time = {11, 25, 0, 0};
    ...
    object_index = Flc_Pass_Schedule_Instance_To_Index(rpdata->object_instance);
    if (object_index >= MAX_FLC_PASS_SCHEDULE)
        return BACNET_STATUS_ERROR;

    apdu = rpdata->application_data;
    switch ((int) rpdata->object_property) {
        case PROP_OBJECT_IDENTIFIER:
            apdu_len = encode_application_object_id(&apdu[0], OBJECT_PASSSCHEDULE,
								rpdata->object_instance + 1);
            break;
        case PROP_OBJECT_NAME:
        case PROP_DESCRIPTION:
            Flc_Pass_Schedule_Object_Name(rpdata->object_instance, &char_string);
            apdu_len = encode_application_character_string(&apdu[0], &char_string);
            break;
        case PROP_OBJECT_TYPE:
            apdu_len = encode_application_enumerated(&apdu[0], OBJECT_PASSSCHEDULE);
            break;
...
bool Flc_Pass_Schedule_Write_Property(
    BACNET_WRITE_PROPERTY_DATA * wp_data)
{
	...
	object_index = Flc_Pass_Schedule_Instance_To_Index(wp_data->object_instance);
	if (object_index >= MAX_FLC_PASS_SCHEDULE) {
        rt_free(value);
		return false;
	}
	...
	switch ((int) wp_data->object_property) {
        case PROP_SCHEDULE:
            ...
            rt_kprintf("pass_schedule write start****************\r\n");
            ...
            rt_kprintf("end**************************************\r\n\r\n");
            app_schedule_write(&schedule, schedule_no, 1, SCHEDULE_TYPE_PASS);
            status = true;
            break;
		case PROP_OBJECT_IDENTIFIER:
		case PROP_OBJECT_NAME:
