...
#if (MAX_TSM_TRANSACTIONS)      /* 原为255，改成5 */
...
static BACNET_TSM_DATA *TSM_List;
...
bool tsm_init(void)
{
    if(TSM_List == RT_NULL)
    {
        TSM_List = rt_malloc(sizeof(BACNET_TSM_DATA) * MAX_TSM_TRANSACTIONS);
        if(TSM_List == RT_NULL)
            return false;
    }
    return true;
    ...
    void)
{
    ...
    if (tsm_transaction_available()) {  /* 是否有空闲队列 */
        while (!found) {
            index = tsm_find_invokeID_index(Current_Invoke_ID);
            if (index == MAX_TSM_TRANSACTIONS) {    /* 当前 invokeID空闲 */
                ...
                if (index != MAX_TSM_TRANSACTIONS) {
                    ...
                    TSM_List[index].RequestTimer = apdu_timeout();      /* 获取溢出时间 */
    ...
    uint16_t milliseconds)
{
    ...
    for (i = 0; i < MAX_TSM_TRANSACTIONS; i++) {
        if (TSM_List[i].state == TSM_STATE_AWAIT_CONFIRMATION) {
            ...
            /* AWAIT_CONFIRMATION */
            if (TSM_List[i].RequestTimer == 0) {        /* 时间到，发送一次数据 */
