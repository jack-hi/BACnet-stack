...
int bacapp_decode_application_data_complex(
    uint8_t * apdu,
    unsigned max_apdu_len,
    BACNET_APPLICATION_DATA_VALUE * value, 
    uint32_t size,
    uint32_t *left)      /* size为最多读取的value数目 */
{
    int len = 0;
    int tag_len = 0;
    int decode_len = 0;
    int tag_count = 0;          /* 记录opentag和closetag的数量，处理完成后应该相等 */
    int tag_check = 0;          /* 检查tag */
    uint8_t tag_number = 0;
    uint32_t len_value_type = 0;
    int left_len = max_apdu_len;
    ...
    /* 检查第一个字节是否为open_tag */
    if(IS_CONTEXT_SPECIFIC(*apdu))      /* bit3 */
    {
        if(IS_OPENING_TAG(*apdu))       /* ((x & 0x07) == 6) */
        {
            tag_check = 1;
        }
    }
    ...
    while(left_len > 0)
    {
        if(IS_CONTEXT_SPECIFIC(*apdu))      /* bit3 */
        {
            ...
            else if(IS_CLOSING_TAG(*apdu))  /* ((x & 0x07) == 7) */
            {
                tag_count--;
                if(tag_check)
                {
        ...
        else
        {
            ...
            value->context_specific = false;
            value->next = NULL;
            tag_len = decode_tag_number_and_value(apdu, &tag_number,
                        &len_value_type);
